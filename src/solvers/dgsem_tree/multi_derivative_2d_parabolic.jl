...
function rhs_mdrk2!(du, u,
    ...
    # Convert conservative variables to a form more suitable for viscous flux calculations
    @trixi_timeit timer() "transform variables" transform_variables!(u_transformed, us,
                                                                     mesh,
                                                                     equations_parabolic,
                                                                     dg, parabolic_scheme,
                                                                     cache, cache_parabolic)
...
@inline function mdrk_kernel_1!(du, flux_viscous, gradients, u_transformed,
                                u, t, dt, tolerances,
                                mesh::TreeMesh{2},
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for jj in eachnode(dg)
            ...
            multiply_add_to_node_vars!(ut, -dt * derivative_matrix[jj, j], flux2, equations,
                                       dg, i, jj)
        end
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        (fmma, gmma), (fmmv, gmmv) = fluxes(umm, (ummx, ummy), equations,
                                            equations_parabolic)
        (fppa, gppa), (fppv, gppv) = fluxes(upp, (uppx, uppy), equations,
                                            equations_parabolic)

        fa, ga = 2 * get_node_vars(Fa, equations, dg, i, j),
                 2 * get_node_vars(Ga, equations, dg, i, j)
        fv, gv = 2 * get_node_vars(Fv, equations, dg, i, j),
                 2 * get_node_vars(Gv, equations, dg, i, j)
        f = fa - fv
        g = ga - gv
        ...
        set_node_vars!(cache_parabolic.mdrk_cache.Fv2, Fv_node2, equations, dg, 1, i, j,
                       element)
        set_node_vars!(mdrk_cache.F2, Ga_node2, equations, dg, 2, i, j, element)
        set_node_vars!(cache_parabolic.mdrk_cache.Fv2, Gv_node2, equations, dg, 2, i, j,
                       element)
...
function calc_volume_integral_mdrk1!(du, flux_viscous, gradients, u_transformed, u, t, dt,
                                     tolerances::NamedTuple,
                                     mesh::TreeMesh{2},
...
function calc_volume_integral_mdrk2!(du, flux_viscous, gradients, u_transformed, u, t, dt,
                                     tolerances::NamedTuple,
                                     mesh::TreeMesh{2},
                                     have_nonconservative_terms, source_terms,
                                     equations,
...
@inline function mdrk_kernel_2!(du, flux_viscous, gradients, u_transformed,
                                us, t, dt, tolerances,
                                mesh::TreeMesh{2},
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(ust, -dt * derivative_matrix[jj, j], flux2,
                                       equations, dg, i, jj)
        end
    end
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for ii in eachnode(dg)
            # ut              += -lam * D * f for each variable
            # i.e.,  ut[ii,j] += -lam * Dm[ii,i] f[i,j] (sum over i)
            multiply_add_to_node_vars!(ustx, derivative_matrix[ii, i], ust_node, equations,
                                       dg,
                                       ii, j)
        end
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(usty, derivative_matrix[jj, j], ust_node, equations,
                                       dg,
                                       i, jj)
        end
    end
