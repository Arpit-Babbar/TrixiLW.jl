...
@inline function mdrk_kernel_1!(
   ...
   utx, uty, uttx, utty, utttx, uttty, uttttx, utttty, upx, upy, umx, umy, uppx, uppy,
   ummx, ummy, fv, gv, Fv, Gv = cache_parabolic.lw_res_cache.cell_arrays[id]
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      Trixi.set_node_vars!(Fa, 0.5*flux_adv_1, equations, dg, i, j)
      Trixi.set_node_vars!(Ga, 0.5*flux_adv_2, equations, dg, i, j)
      Trixi.set_node_vars!(Fv, 0.5*flux_visc_1, equations, dg, i, j)
      Trixi.set_node_vars!(Gv, 0.5*flux_visc_2, equations, dg, i, j)
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      for ii in eachnode(dg)
         # res              += -lam * D * F for each variable
         # i.e.,  res[ii,j] += -lam * Dm[ii,i] F[i,j] (sum over i)U_node
         multiply_add_to_node_vars!(du, 1 * derivative_dhat[ii, i], F_node,
      ...
      for jj in eachnode(dg)
         # C += -lam*g*Dm' for each variable
         # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
         multiply_add_to_node_vars!(du, 1 * derivative_dhat[jj, j], G_node,
...
@inline function mdrk_kernel_2!(
   ...
   fa, ga, Fa, Ga, ut, ust, uttt, U, up, upp, um, umm, S = cell_arrays[id]

   utx, uty, ustx, usty, uttx, utty, upx, upy, umx, umy, uppx, uppy,
   ummx, ummy, fv, gv, Fv, Gv = cache_parabolic.lw_res_cache.cell_arrays[id]
   refresh!.((ut, ust, ustx, usty, utx, uty))
   for j in eachnode(dg), i in eachnode(dg)
      u_node = Trixi.get_node_vars(u, equations, dg, i, j, element)
      ...
      flux_adv_1_s, flux_adv_2_s = fluxes(us_node, equations)
      flux_visc_1 = Trixi.get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j, element)
      flux_visc_2 = Trixi.get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j, element)
      flux1 = flux_adv_1 - flux_visc_1
      flux2 = flux_adv_2 - flux_visc_2
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      for ii in eachnode(dg)
         ...
         Trixi.multiply_add_to_node_vars!(utx, derivative_matrix[ii, i], ut_node, equations, dg,
            ii, j)
         Trixi.multiply_add_to_node_vars!(ustx, derivative_matrix[ii, i], ust_node, equations, dg,
            ii, j)
      end
      ...
      for jj in eachnode(dg)
         ...
         Trixi.multiply_add_to_node_vars!(uty, derivative_matrix[jj, j], ut_node, equations, dg,
            i, jj)
         Trixi.multiply_add_to_node_vars!(usty, derivative_matrix[jj, j], ust_node, equations, dg,
            i, jj)
      end
   end
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      us_node = Trixi.get_node_vars(us, equations, dg, i, j, element)
      ust_node = Trixi.get_node_vars(ust, equations, dg, i, j)
      ustx_node = Trixi.get_node_vars(ustx, equations, dg, i, j)
      usty_node = Trixi.get_node_vars(usty, equations, dg, i, j)
