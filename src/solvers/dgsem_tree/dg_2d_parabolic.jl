...
@muladd begin
    ...
    function rhs!(du, u, t, dt, mesh::Union{TreeMesh{2}, P4estMesh{2}}, equations,
        ...
        @trixi_timeit timer() "prolong2interfaces" prolong2interfaces_lw_parabolic!(cache,
                                                                                    cache_parabolic,
                                                                                    u, mesh,
                                                                                    equations,
                                                                                    dg.surface_integral,
        ...
        @trixi_timeit timer() "interface flux" calc_interface_flux_hyperbolic_parabolic!(cache.elements.surface_flux_values,
                                                                                         mesh,
                                                                                         # have_nonconservative_terms(equations),
                                                                                         equations,
                                                                                         equations_parabolic,
        ...
        @trixi_timeit timer() "prolong2boundaries" prolong2boundaries_visc_lw!(cache_parabolic,
                                                                               flux_viscous,
                                                                               mesh,
                                                                               equations_parabolic,
                                                                               dg.surface_integral,
        ...
        @trixi_timeit timer() "boundary flux" calc_boundary_flux_divergence_lw!(cache_parabolic,
                                                                                cache, t,
                                                                                boundary_conditions_parabolic,
                                                                                mesh,
                                                                                equations_parabolic,
    ...
    function create_cache(mesh::Union{TreeMesh{2}, P4estMesh{2}},
                          ...
                          time_discretization::AbstractLWTimeDiscretization, dg, RealT,
                          uEltype, cache)
    ...
    fluxes(u, grad_u::Tuple{<:Any, <:Any}, equations_parabolic::AbstractEquationsParabolic{2}) = (Trixi.flux(u,
                                                                                                             grad_u,
                                                                                                             1,
                                                                                                             equations_parabolic),
                                                                                                  Trixi.flux(u,
    ...
    function calc_volume_integral!(du, flux_viscous, gradients, u_transformed, u, t, dt,
    ...
    fluxes(u, grad_u, equations::AbstractEquations{2}, equations_parabolic::AbstractEquationsParabolic{2}) = (Trixi.flux(u,
                                                                                                                         1,
                                                                                                                         equations),
                                                                                                              Trixi.flux(u,
                                                                                                                         2,
    ...
    function lw_volume_kernel_1!(du, flux_viscous, gradients, u_transformed, u, t, dt,
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            flux_visc_1 = get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j,
                                        element)
            flux_visc_2 = get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j,
                                        element)
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                # ut              += -lam * D * f for each variable
                # i.e.,  ut[ii,j] += -lam * Dm[ii,i] f[i,j] (sum over i)
                multiply_add_to_node_vars!(utx, derivative_matrix[ii, i], ut_node,
                                           equations, dg,
                                           ii, j)
            end
            ...
            for jj in eachnode(dg)
                # C += -lam*g*Dm' for each variable
                # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
                multiply_add_to_node_vars!(uty, derivative_matrix[jj, j], ut_node,
                                           equations, dg,
                                           i, jj)
            end
        end
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            (fma, gma), (fmv, gmv) = fluxes(um_node, (umx_node, umy_node), equations,
                                            equations_parabolic)
            (fpa, gpa), (fpv, gpv) = fluxes(up_node, (upx_node, upy_node), equations,
                                            equations_parabolic)
    ...
    function lw_volume_kernel_2!(du, flux_viscous, gradients, u_transformed, u, t, dt,
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                # ut              += -lam * D * f for each variable
                # i.e.,  ut[ii,j] += -lam * Dm[ii,i] f[i,j] (sum over i)
                multiply_add_to_node_vars!(uttx, derivative_matrix[ii, i], utt_node,
                                           equations, dg,
                                           ii, j)
            end
            ...
            for jj in eachnode(dg)
                # C += -lam*g*Dm' for each variable
                # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
                multiply_add_to_node_vars!(utty, derivative_matrix[jj, j], utt_node,
                                           equations, dg,
                                           i, jj)
            end
        end
    ...
    function lw_volume_kernel_3!(du, flux_viscous, gradients, u_transformed, u, t, dt,
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            (fmma, gmma), (fmmv, gmmv) = fluxes(umm_node, (ummx_node, ummy_node), equations,
                                                equations_parabolic)
            (fppa, gppa), (fppv, gppv) = fluxes(upp_node, (uppx_node, uppy_node), equations,
                                                equations_parabolic)
    ...
    function lw_volume_kernel_4!(du, flux_viscous, gradients, u_transformed, u, t, dt,
        ...
        refresh!.((ut, utx, uty, utt, uttx, utty, uttt, utttx, uttty, utttt, uttttx,
                   utttty))
        ...
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for ii in eachnode(dg)
                # ut              += -lam * D * f for each variable
                # i.e.,  ut[ii,j] += -lam * Dm[ii,i] f[i,j] (sum over i)
                multiply_add_to_node_vars!(uttttx, derivative_matrix[ii, i], utttt_node,
                                           equations, dg,
                                           ii, j)
            end
            ...
            for jj in eachnode(dg)
                # C += -lam*g*Dm' for each variable
                # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
                multiply_add_to_node_vars!(utttty, derivative_matrix[jj, j], utttt_node,
                                           equations, dg,
                                           i, jj)
            end
        end
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            if orientations[interface] == 1
                ...
                for j in eachnode(dg), v in eachvariable(equations)
                    ...
                    interface_cache.U[1, v, j, interface] = U[v, nnodes(dg), j,
                                                              left_element]
                    interface_cache.U[2, v, j, interface] = U[v, 1, j, right_element]

                    # Fluxes
                    interface_cache.f[1, v, j, interface] = F[v, 1, nnodes(dg), j,
                                                              left_element]
                    interface_cache.f[2, v, j, interface] = F[v, 1, 1, j, right_element]

                    cache_parabolic.Fb[1, v, j, interface] = Fv[v, 1, nnodes(dg), j,
                                                                left_element]
                ...
                for i in eachnode(dg), v in eachvariable(equations)
                    ...
                    interface_cache.U[1, v, i, interface] = U[v, i, nnodes(dg),
                                                              left_element]
                    interface_cache.U[2, v, i, interface] = U[v, i, 1, right_element]

                    # Fluxes
                    interface_cache.f[1, v, i, interface] = F[v, 2, i, nnodes(dg),
                                                              left_element]
                    interface_cache.f[2, v, i, interface] = F[v, 2, i, 1, right_element]

                    cache_parabolic.Fb[1, v, i, interface] = Fv[v, 2, i, nnodes(dg),
                                                                left_element]
    ...
    function calc_interface_flux_hyperbolic_parabolic!(surface_flux_values,
                                                       mesh::TreeMesh{2},
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for i in eachnode(dg)
                ...
                U_ll, U_rr = get_surface_node_vars(interface_cache.U, equations, dg, i,
                                                   interface)
                u_ll, u_rr = get_surface_node_vars(u, equations, dg, i, interface)
                f_ll, f_rr = get_surface_node_vars(interface_cache.f, equations, dg, i,
                                                   interface)
                flux_hyperbolic = surface_flux(f_ll, f_rr, u_ll, u_rr, U_ll, U_rr,
                                               orientations[interface], equations)
                f_visc_ll, f_visc_rr = get_surface_node_vars(cache_parabolic.Fb, equations,
                                                             dg, i, interface)
                flux_parabolic = 0.5 * (f_visc_ll + f_visc_rr)
                # Copy flux to left and right element storage
                for v in eachvariable(equations)
                    surface_flux_values[v, i, left_direction, left_id] = flux_hyperbolic[v] -
                                                                         flux_parabolic[v]
                    surface_flux_values[v, i, right_direction, right_id] = flux_hyperbolic[v] -
                                                                           flux_parabolic[v]
                end
            end
        end
    ...
    function calc_boundary_flux_by_direction_divergence_lw!(surface_flux_values::AbstractArray{<:Any,
                                                                                               4},
                                                            t,
                                                            boundary_condition,
                                                            equations_parabolic::AbstractEquationsParabolic,
        ...
        @threaded for boundary in first_boundary:last_boundary
            ...
            for i in eachnode(dg)
                ...
                flux = boundary_condition(flux_inner, u_inner, gradient, outer_cache,
                                          ...
                                          x, t, dt, dg, Divergence(), equations_parabolic,
