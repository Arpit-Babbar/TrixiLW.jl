...
function create_cache_serial(mesh::Union{TreeMesh, StructuredMesh, UnstructuredMesh2D,
                                         P4estMesh},
    ...
    N4() = NamedTuple{(:f, :g, :ftilde, :gtilde, :Ftilde, :Gtilde, :ut, :utt, :uttt, :utttt,
                       :U, :up, :um, :upp, :umm, :S, :u_np1, :u_np1_low)}((MArr(undef)
                                                                           for _ in 1:18))
...
function create_cache(mesh::Union{TreeMesh{NDIMS, <:Trixi.SerialTree{NDIMS}},
                                  ...
                                  P4estMesh{<:Any, <:Any, <:Any}} where {NDIMS},
                      equations::AbstractEquations,
                      time_discretization::AbstractLWTimeDiscretization,
                      dg, RealT, uEltype, cache)
    return create_cache_serial(mesh, equations, time_discretization, dg, RealT, uEltype,
                               cache)
end
...
function create_element_cache(::Union{TreeMesh, StructuredMesh, UnstructuredMesh2D,
                                      P4estMesh},
    ...
    if isa(time_discretization, MDRK)
        ...
        mdrk_cache = MDRKElementCache(us, u_np1, u_np1_low, U2, S2, F, F2, _us, _u_np1,
                                      _u_np1_low, _U2, _S2, _F, _F2)
    else
        mdrk_cache = (;)
    end
    fn_low = unsafe_wrap(Array{uEltype, NDIMS + 2}, pointer(_fn_low),
                         (n_variables, ntuple(_ -> n_nodes, NDIMS - 1)..., 2^NDIMS,
                          n_elements))
...
function create_mortar_cache(mesh::TreeMesh, equations, dg, uEltype, RealT,
    ...
    _U_upper, _U_lower, _F_upper, _F_lower, _fn_low_upper, _fn_low_lower = (copy(_u_upper) for _ in 1:6)

    wrap_(u) = unsafe_wrap(Array, pointer(u), size(u_upper))
    U_upper, U_lower, F_upper, F_lower, fn_low_upper, fn_low_lower = wrap_.((_U_upper,
                                                                             _U_lower,
                                                                             _F_upper,
                                                                             _F_lower,
                                                                             _fn_low_upper,
    ...
    L2MortarContainer_lw_Tree(U_upper, U_lower, F_upper, F_lower, fn_low_upper,
                              fn_low_lower,
...
function ninterfaces(mesh::Union{TreeMesh, UnstructuredMesh2D, P4estMesh}, dg, cache,
                     ::AbstractTimeDiscretization) # Add RK Type here
    return ninterfaces(dg, cache)
end

function nboundaries(mesh::Union{TreeMesh, UnstructuredMesh2D, P4estMesh}, dg, cache,
                     ::AbstractTimeDiscretization) # Add RK Type here
