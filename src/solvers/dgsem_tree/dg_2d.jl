...
function rhs!(du, u, t, dt, mesh::Union{TreeMesh{2}, P4estMesh{2}}, equations,
    ...
    @trixi_timeit timer() "volume integral" calc_volume_integral!(du, u, t, dt,
                                                                  tolerances, mesh,
    ...
    @trixi_timeit timer() "boundary flux" calc_boundary_flux!(cache, t, dt,
...
@inline function fv_kernel!(du, u, dt, ::MUSCLHancockReconstruction,
                            ...
                            nonconservative_terms, equations,
                            volume_flux_fv, dg::DGSEM, cache, element, alpha = true)
    @unpack fstar1_L_threaded, fstar1_R_threaded, fstar2_L_threaded, fstar2_R_threaded,     # unph_threaded, uf_threaded
    mh_face_arrays,
    uext_threaded = cache
...
fluxes(u, equations::AbstractEquations{2}) = (Trixi.flux(u, 1, equations),
...
@inline function lw_volume_kernel_1!(du, u,
                                     ...
                                     dg::DGSEM{<:Any, <:Any, <:Any,
                                               <:Union{VolumeIntegralFR{LW},
                                                       VolumeIntegralFRShockCapturing{LW,
                                                                                      <:Any}}},
                                     cache, alpha = true)
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        # inv_jacobian = inverse_jacobian[i, j, element]
        multiply_add_to_node_vars!(du, -1.0 / inv_jacobian, S_node, equations, dg, i, j,
                                   element)
    end
...
@inline function lw_volume_kernel_2!(du, u,
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for ii in eachnode(dg)
            # ut              += -lam * D * f for each variable
            # i.e.,  ut[ii,j] += -lam * Dm[ii,i] f[i,j] (sum over i)
            multiply_add_to_node_vars!(ut, derivative_matrix[ii, i], flux1, equations,
                                       dg, ii, j)
        end
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(ut, derivative_matrix[jj, j], flux2, equations,
                                       dg, i, jj)
        end
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for ii in eachnode(dg)
            # res              += -lam * D * F for each variable
            # i.e.,  res[ii,j] += -lam * Dm[ii,i] F[i,j] (sum over i)U_node
            multiply_add_to_node_vars!(utt, derivative_matrix[ii, i], ft_node,
                                       equations, dg, ii, j)
        end
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(utt, derivative_matrix[jj, j], gt_node,
                                       equations, dg, i, jj)
        end
    end
...
@inline function lw_volume_kernel_2!(du, u,
    ...
    f, g, F, G, ut, utt, U, S = cell_arrays[id]
    refresh!.((ut, utt))
    for j in eachnode(dg), i in eachnode(dg)
        u_node = get_node_vars(u, equations, dg, i, j, element)
...
@inline function lw_volume_kernel_4!(du, u,
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        for ii in eachnode(dg)
            # res              += -lam * D * F for each variable
            # i.e.,  res[ii,j] += -lam * Dm[ii,i] F[i,j] (sum over i)
            multiply_add_to_node_vars!(utt, -dt * derivative_matrix[ii, i], f_t,
                                       equations, dg, ii, j)
        end
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(utt, -dt * derivative_matrix[jj, j], g_t,
                                       equations, dg, i, jj)
        end
    end
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        f_node, g_node = get_node_vars(f, equations, dg, i, j),
                         get_node_vars(g, equations, dg, i, j)
        um_node = get_node_vars(um, equations, dg, i, j)
        up_node = get_node_vars(up, equations, dg, i, j)
        umm_node = get_node_vars(umm, equations, dg, i, j)
        upp_node = get_node_vars(upp, equations, dg, i, j)
        ...
        for ii in eachnode(dg)
            # res              += -lam * D * F for each variable
            # i.e.,  res[ii,j] += -lam * Dm[ii,i] F[i,j] (sum over i)
            multiply_add_to_node_vars!(uttt, -dt * derivative_matrix[ii, i], f_tt,
                                       equations, dg, ii, j)
        end
        ...
        for jj in eachnode(dg)
            # C += -lam*g*Dm' for each variable
            # C[i,jj] += -lam*g[i,j]*Dm[jj,j] (sum over j)
            multiply_add_to_node_vars!(uttt, -dt * derivative_matrix[jj, j], g_tt,
                                       equations, dg, i, jj)
        end
    end
    ...
    @threaded for interface in eachinterface(dg, cache)
        ...
        if orientations[interface] == 1
            ...
            for j in eachnode(dg), v in eachvariable(equations)
                ...
                interface_cache.U[1, v, j, interface] = element_cache.U[v, nnodes(dg),
                                                                        j, left_element]
                interface_cache.U[2, v, j, interface] = element_cache.U[v, 1, j,
                                                                        right_element]

                # Fluxes
                # TODO - RENAME THIS TO F ALREADY!!
                interface_cache.f[1, v, j, interface] = element_cache.F[v, 1,
                                                                        nnodes(dg), j,
                                                                        left_element]
                interface_cache.f[2, v, j, interface] = element_cache.F[v, 1, 1, j,
                                                                        right_element]
            end
            for v in eachvariable(equations), j in eachnode(dg)
                interface_cache.fn_low[1, v, j, interface] = element_cache.fn_low[v, j,
                                                                                  2,
                                                                                  left_element]
                interface_cache.fn_low[2, v, j, interface] = element_cache.fn_low[v, j,
                                                                                  1,
                                                                                  right_element]
            end
            ...
            for i in eachnode(dg), v in eachvariable(equations)
                ...
                # Time averaged solution
                interface_cache.U[1, v, i, interface] = element_cache.U[v, i,
                                                                        nnodes(dg),
                                                                        left_element]
                interface_cache.U[2, v, i, interface] = element_cache.U[v, i, 1,
                                                                        right_element]

                # Fluxes
                interface_cache.f[1, v, i, interface] = element_cache.F[v, 2, i,
                                                                        nnodes(dg),
                                                                        left_element]
                interface_cache.f[2, v, i, interface] = element_cache.F[v, 2, i, 1,
                                                                        right_element]
            end
            for v in eachvariable(equations), i in eachnode(dg)
                interface_cache.fn_low[1, v, i, interface] = element_cache.fn_low[v, i,
                                                                                  4,
                                                                                  left_element]
                interface_cache.fn_low[2, v, i, interface] = element_cache.fn_low[v, i,
                                                                                  3,
                                                                                  right_element]
            end
        end
        ...
        if orientations[boundary] == 1
            ...
            if neighbor_sides[boundary] == 1
                ...
                for l in eachnode(dg), v in eachvariable(equations)
                    ...
                    boundary_cache.f[1, v, l, boundary] = F[v, 1, nnodes(dg), l,
                                                            element]
                end
            ...
            if neighbor_sides[boundary] == 1
                ...
                for l in eachnode(dg), v in eachvariable(equations)
                    ...
                    boundary_cache.f[1, v, l, boundary] = F[v, 2, l, nnodes(dg),
                                                            element]
                end
            else
                # element in +y direction of boundary
...
function calc_boundary_flux!(cache, t, dt,
...
function calc_boundary_flux!(cache, t, dt, boundary_conditions::NamedTuple,
    ...
    calc_adv_boundary_flux_by_direction!(surface_flux_values, t, dt,
...
function calc_adv_boundary_flux_by_direction!(surface_flux_values::AbstractArray{<:Any,
                                                                                 4}, t,
                                              dt,
                                              boundary_condition, equations,
                                              surface_integral,
                                              ::AbstractLWTimeDiscretization, dg::DG,
                                              cache,
    ...
    @threaded for boundary in first_boundary:last_boundary
        ...
        for i in eachnode(dg)
            ...
            flux = boundary_condition(U_inner, F_inner, u_inner, outer_cache,
...
function prolong2mortars!(cache, u,
                          ...
                          time_discretization::AbstractLWTimeDiscretization, dg::DGSEM)
    ...
    @threaded for mortar in eachmortar(dg, cache)
            ...
            if cache.mortars.orientations[mortar] == 1
                ...
                for l in eachnode(dg)
                    for v in eachvariable(equations)
                        ...
                        lw_mortars.F_upper[2, v, l, mortar] = F[v, 1, 1, l,
                                                                upper_element]
                        lw_mortars.F_lower[2, v, l, mortar] = F[v, 1, 1, l,
                                                                lower_element]
                    end
                end
            else
                ...
                for l in eachnode(dg)
                    for v in eachvariable(equations)
                        ...
                        lw_mortars.F_upper[2, v, l, mortar] = F[v, 2, l, 1,
                                                                upper_element]
                        lw_mortars.F_lower[2, v, l, mortar] = F[v, 2, l, 1,
                                                                lower_element]
                    end
                end
            end
            ...
            if cache.mortars.orientations[mortar] == 1
                ...
                for l in eachnode(dg)
                    for v in eachvariable(equations)
                        ...
                        lw_mortars.U_upper[1, v, l, mortar] = U[v, nnodes(dg), l,
                                                                upper_element]
                        lw_mortars.U_lower[1, v, l, mortar] = U[v, nnodes(dg), l,
                                                                lower_element]

                        lw_mortars.F_upper[1, v, l, mortar] = F[v, 1, nnodes(dg), l,
                                                                upper_element]
                        lw_mortars.F_lower[1, v, l, mortar] = F[v, 1, nnodes(dg), l,
                                                                lower_element]
                    end
                end
            else
                ...
                for l in eachnode(dg)
                    for v in eachvariable(equations)
                        ...
                        lw_mortars.U_upper[1, v, l, mortar] = U[v, l, nnodes(dg),
                                                                upper_element]
                        lw_mortars.U_lower[1, v, l, mortar] = U[v, l, nnodes(dg),
                                                                lower_element]

                        lw_mortars.F_upper[1, v, l, mortar] = F[v, 2, l, nnodes(dg),
                                                                upper_element]
                        lw_mortars.F_lower[1, v, l, mortar] = F[v, 2, l, nnodes(dg),
                                                                lower_element]
                    end
                end
            end
        ...
        if cache.mortars.large_sides[mortar] == 1 # -> large element on left side
            ...
            if cache.mortars.orientations[mortar] == 1
                ...
                element_solutions_to_mortars!(cache.mortars, cache.lw_mortars,
                                              mortar_l2,
                                              leftright, mortar, u_large, U_large,
                                              F_large,
                                              time_discretization)
            else
                # L2 mortars in y-direction
...
@inline function element_solutions_to_mortars!(mortars, lw_mortars,
                                               mortar_l2::LobattoLegendreMortarL2,
                                               leftright, mortar,
                                               u_large::AbstractArray{<:Any, 2},
                                               U_large::AbstractArray{<:Any, 2},
                                               F_large::AbstractArray{<:Any, 2},
                                               time_discretization::AbstractLWTimeDiscretization)
...
@inline function calc_fstar!(destination::AbstractArray{<:Any, 2}, equations,
                             ...
                             time_discretization::AbstractLWTimeDiscretization,
                             dg::DGSEM,
                             u_interfaces, F_interfaces, U_interfaces, interface,
                             orientation)
    for i in eachnode(dg)
...
function compute_subcells(::Union{TreeMesh{2}, StructuredMesh{2}, UnstructuredMesh2D,
                                  P4estMesh{2}},
                          dg::DG, RealT = eltype(dg.basis.weights))
    @unpack nodes, weights = dg.basis
    # subfaces in reference cell
    x_subfaces, y_subfaces = (OffsetArray(Vector{RealT}(undef, nnodes(dg) + 1),
    ...
    ξ_extended = OffsetArray(Vector{RealT}(undef, nnodes(dg) + 2),
                             OffsetArrays.Origin(0))
...
function create_cache(mesh::Union{TreeMesh{2}, StructuredMesh{2}, UnstructuredMesh2D,
    ...
    constructor = x -> abstract_constructor(Tuple{nvar, nnodes(dg) + 2, nnodes(dg) + 2},
                                            x, (1, 0, 0))
    ...
    mh_face_arrays = [(zeros(nvar, 4, nnodes(dg), nnodes(dg)),
...
function compute_alp(u_ll, u_rr, primary_element_index, secondary_element_index, Jl, Jr,
                     ...
                     dg, volume_integral::VolumeIntegralFR, mesh::TreeMesh)
    return zero(eltype(u_ll))
end

function compute_alp(u_ll, u_rr, primary_element_index, secondary_element_index, Jl, Jr,
                     dt,
                     fn, Fn_, fn_inner_ll, fn_inner_rr, primary_node_index, equations,
                     dg, volume_integral::VolumeIntegralFRShockCapturing,
                     mesh::TreeMesh)
...
function calc_interface_flux!(surface_flux_values, mesh::TreeMesh{2},
    ...
    @threaded for interface in eachinterface(dg, cache)
        ...
        for i in eachnode(dg)
            ...
            fn_inner_ll, fn_inner_rr = get_surface_node_vars(fn_low, equations, dg, i,
                                                             interface)
            Fn_ = surface_flux(f_ll, f_rr, u_ll, u_rr, U_ll, U_rr,
