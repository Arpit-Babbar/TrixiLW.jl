using Trixi: SemidiscretizationHyperbolic, SemidiscretizationHyperbolicParabolic,
             ...
             DiscreteCallback, compute_coefficients!, u_modified!, max_level
...
mutable struct LWIntegrator{SemiDiscretization,
                            uType,
                            Solution <: LWSolution,
                            CacheType,
                            tType,
...
function initialize_callbacks!(callbacks::NTuple{N, Any},
                               integrator::LWIntegrator) where {N}
...
function initialize!(cb::DiscreteCallback{Condition, Affect!}, u, t,
    ...
    @trixi_timeit timer() "initial condition AMR" if amr_callback.adapt_initial_condition
        ...
        # TODO - THIS NEEDS TO BE UNCOMMENTED TO BE ABLE TO SEE THE CONSERVATION PROPERTY
...
function compute_dt(semi::SemidiscretizationHyperbolic,
                    mesh::Union{ParallelTreeMesh, TreeMesh, StructuredMesh,
    ...
    dt = Trixi.@trixi_timeit timer() "calculate dt" cfl_number*max_dt(u, t, mesh,
...
function compute_dt(semi::SemidiscretizationHyperbolicParabolic,
                    mesh::Union{ParallelTreeMesh, TreeMesh, P4estMesh},
    ...
    dt_adv = Trixi.@trixi_timeit timer() "calculate dt" cfl_number*max_dt(u, t, mesh,
                                                                          have_constant_speed(equations),
                                                                          equations, solver,
                                                                          cache)

    dt_visc = Trixi.@trixi_timeit timer() "calculate dt" cfl_number*max_dt(u, t, mesh,
                                                                           equations_parabolic,
                                                                           solver, cache)
