using Trixi: AbstractVolumeIntegral, DG, summary_header, summary_line,
             ...
             AbstractIndicator, False, get_node_vars, set_node_vars!,
...
@muladd begin
    struct VolumeIntegralFR{TimeDiscretization <: AbstractTimeDiscretization} <:
    ...
    function Base.show(io::IO, mime::MIME"text/plain",
                       dg::DG{<:Any, <:Any, <:Any, <:VolumeIntegralFR})
        @nospecialize dg # reduce precompilation time
    ...
    struct VolumeIntegralFRShockCapturing{TimeDiscretization, VolumeFluxFV, Indicator,
                                          Reconstruction} <: AbstractVolumeIntegral
    ...
    get_time_discretization(volume_integral::VolumeIntegralFRShockCapturing) = volume_integral.volume_integralFR.time_discretization
    ...
    function VolumeIntegralFRShockCapturing(indicator;
                                            volume_integralFR = VolumeIntegralFR(LW()),
                                            volume_flux_fv = flux_lax_friedrichs,
                                            reconstruction = FirstOrderReconstruction())
        return VolumeIntegralFRShockCapturing{typeof(get_time_discretization(volume_integralFR)),
                                              typeof(volume_flux_fv),
                                              typeof(indicator), typeof(reconstruction)}(volume_integralFR,
                                                                                         volume_flux_fv,
                                                                                         indicator,
                                                                                         reconstruction)
    end
    ...
    function get_element_variables!(element_variables, u, mesh, equations,
                                    volume_integral::VolumeIntegralFRShockCapturing, dg,
                                    cache)
    ...
    @inline function get_contravariant_matrix(contravariant_vectors::AbstractArray,
                                              indices...)
        Ja1 = SVector(ntuple(@inline(dim->contravariant_vectors[dim, 1, indices...]),
                             Val(ndims(contravariant_vectors) - 3)))
        Ja2 = SVector(ntuple(@inline(dim->contravariant_vectors[dim, 2, indices...]),
                             Val(ndims(contravariant_vectors) - 3)))
        return Ja1, Ja2
    end
    ...
    function normal_product(F, equations, normal)
        ...
        Fn = SVector(ntuple(@inline(v->F[1][v] * normal[1] + F[2][v] * normal[2]),
