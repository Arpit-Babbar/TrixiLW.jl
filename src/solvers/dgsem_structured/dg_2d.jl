...
function finite_differences_(h1, h2, ul, u, ur)
   if min(h1, h2) < 1e-12
...
@noinline function calcflux_muscl!(fstar1_L, fstar1_R, fstar2_L, fstar2_R,
   uf, uext, alpha, u, Δt,
   mesh::Union{StructuredMesh{2},UnstructuredMesh2D,P4estMesh{2}},
   nonconservative_terms::False, equations,
   volume_flux_fv, dg::DGSEM, element, cache)
   ...
   @turbo uext .= convert(Float64, NaN)
   @turbo uf   .= convert(Float64, NaN)
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      back_x, cent_x, fwd_x = finite_differences_(Δx1, Δx2, ul, u_, ur)
      back_y, cent_y, fwd_y = finite_differences_(Δy1, Δy2, ud, u_, uu)

      beta1, beta2 = 2.0 - alpha, 2.0 - alpha # Unfortunate way to fix type instability
      # beta1 = beta2 = 1.0

      slope_tuple_x = (minmod(cent_x[n], back_x[n], fwd_x[n], beta1, 0.0)
                       for n in eachvariable(equations))
      # slope_tuple_x = (minmod(back_x[n], fwd_x[n]) for n in eachvariable(equations))
      slope_x = SVector{nvar}(slope_tuple_x)
      # beta2 = 2.0
      slope_tuple_y = (minmod(cent_y[n], back_y[n], fwd_y[n], beta2, 0.0)
                       for n in eachvariable(equations))
      # slope_tuple_y = (minmod(back_y[n], fwd_y[n]) for n in eachvariable(equations))
      ...
      u_star_l = u_ + 2.0 * slope_x * (x_subfaces[i-1] - ξ_extended[i]) # left face value u_{i-1/2,j}
      u_star_r = u_ + 2.0 * slope_x * (x_subfaces[i]   - ξ_extended[i]) # right face value u_{i+1/2,j}
      ...
      ufl, ufr = limit_slope(equations, slope_x, ufl, u_star_l, ufr, u_star_r, u_,
...
@inline function calcflux_mh!(fstar1_L, fstar1_R, fstar2_L, fstar2_R,
   ...
   for j in eachnode(dg), i in eachnode(dg)
      ...
      # Δx1 = Δx2 = Δy1 = Δy2 = 0.0 # Correctly sets to first order, at least for MUSCL (not tested for MUSCL-Hancock)
      ...
      # @assert isapprox(slope_x, SVector{nvar}(ntuple(_ -> 0.0, nvar)))
      # @assert isapprox(slope_y, SVector{nvar}(ntuple(_ -> 0.0, nvar)))
   ...
   # Ghost values of uf. TODO - Is this okay?
   # For MUSCL, this does not look okay to me. This is basically just forcing the numerical flux
   # to take one value. At the boundary faces, the Riemann solver doesn't matter.
   @views for i in eachnode(dg)
      unph[:, 1:4, i, 0] .= uf[:, 1:4, i, 0] .= uf[:, 1:4, i, 1]
      unph[:, 1:4, i, nd+1] .= uf[:, 1:4, i, nd+1] .= uf[:, 1:4, i, nd]
      unph[:, 1:4, 0, i] .= uf[:, 1:4, 0, i] .= uf[:, 1:4, 1, i]
      unph[:, 1:4, nd+1, i] .= uf[:, 1:4, nd+1, i] .= uf[:, 1:4, nd, i]
   end
   ...
   for j in eachnode(dg)
      ...
      for i in 2:nnodes(dg)
         ...
         u_ll = Trixi.get_node_vars(uf, equations, dg, 2, i - 1, j)
         u_rr = Trixi.get_node_vars(uf, equations, dg, 1, i, j)
         f_ll = Trixi.flux(u_ll, normal_direction, equations)
         f_rr = Trixi.flux(u_rr, normal_direction, equations)
   ...
   for i in eachnode(dg)
      ...
      for j in 2:nnodes(dg)
         ...
         res_dd = -0.5 * Δt * inverse_jacobian[i, j, element] / Δyi * f_ll
         unph[:, 3, i, j-1] .+= res_dd
         unph[:, 4, i, j-1] .+= res_dd
         ...
         if j < nnodes(dg)
            ...
            res_uu = 0.5 * Δt * inverse_jacobian[i, j+1, element] / Δyip1 * f_rr
            unph[:, 3, i, j] .+= res_uu
            unph[:, 4, i, j] .+= res_uu
   ...
   # Set border faces to first order (Not needed for MUSCL)
   for i in eachnode(dg)
      unph[:, 1:2, 0, i] .= @view uf[:, 1:2, 0, i]
      unph[:, 1:2, end, i] .= @view uf[:, 1:2, end, i]
      unph[:, 3:4, i, 0] .= @view uf[:, 3:4, i, 0]
      unph[:, 3:4, i, end] .= @view uf[:, 3:4, i, end]
   end
   ...
   for j in eachnode(dg)
      ...
      for i in 2:nnodes(dg)
         u_ll_ = Trixi.get_node_vars(u, equations, dg, i - 1, j, element)
         u_rr_ = Trixi.get_node_vars(u, equations, dg, i, j, element)
         if i > 1
            u_ll = Trixi.get_node_vars(uf, equations, dg, 2, i - 1, j)
            # @assert u_ll≈u_ll_ u_ll, u_ll_
         else
            u_ll = Trixi.get_node_vars(uext, equations, dg, i - 1, j)
         end

         if i < nnodes(dg)
            u_rr = Trixi.get_node_vars(uf, equations, dg, 1, i, j)
            # @assert u_rr≈u_rr_ u_rr, u_rr_
         else
            u_rr = Trixi.get_node_vars(uext, equations, dg, i, j)
         end
   ...
   for i in eachnode(dg)
      ...
      for j in 2:nnodes(dg)
         u_ll_ = Trixi.get_node_vars(u, equations, dg, i, j - 1, element)
         u_rr_ = Trixi.get_node_vars(u, equations, dg, i, j, element)
         ...
         if j > 1
            ...
            u_ll = Trixi.get_node_vars(uext, equations, dg, i, j - 1)
         end
...
function compute_alp(
   ...
   fn, Fn, fn_inner_ll, fn_inner_rr, primary_node_index, equations, dg, volume_integral::VolumeIntegralFRShockCapturing, mesh::StructuredMesh)
