function rhs_mdrk1!(du, u, t,
    ...
    alpha = @trixi_timeit timer() "volume integral" calc_volume_integral_mdrk1!(du, u, t,
                                                                                dt,
                                                                                tolerances,
                                                                                mesh,
...
function rhs_mdrk2!(du, u,
    ...
    # Calculate volume integral
    alpha = @trixi_timeit timer() "volume integral" calc_volume_integral_mdrk2!(du, u, us,
                                                                                t, dt,
                                                                                tolerances,
                                                                                mesh,
                                                                                have_nonconservative_terms(equations),
    ...
    # Calculate interface fluxes
    @trixi_timeit timer() "interface flux" calc_interface_flux!(cache, u, dt, mesh, # TODO - us or u or both?
                                                                have_nonconservative_terms(equations),
                                                                equations,
                                                                dg.surface_integral,
                                                                time_discretization, alpha,
...
@inline function mdrk_kernel_1!(du, u, t, dt, tolerances, element,
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        set_node_vars!(u_np1_low, u_node, equations, dg, i, j, element)
    end
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        ftilde_node = 2.0 * get_node_vars(Ftilde, equations, dg, i, j) # just cv_f
        gtilde_node = 2.0 * get_node_vars(Gtilde, equations, dg, i, j) # just cv_g
        s_node = 2.0 * get_node_vars(S, equations, dg, i, j)
        Ftilde_node_low = ftilde_node + 0.25 * ftilde_t
        Gtilde_node_low = gtilde_node + 0.25 * gtilde_t
        S_node_low = s_node + 0.25 * st

        multiply_add_to_node_vars!(element_cache.F, 0.125, ft, equations, dg, 1, i, j,
                                   element)
        multiply_add_to_node_vars!(element_cache.F, 0.125, gt, equations, dg, 2, i, j,
                                   element)
        ...
        multiply_add_to_node_vars!(mdrk_cache.F2, 1.0 / 6.0, ft, equations, dg, 1, i, j,
                                   element)
        multiply_add_to_node_vars!(mdrk_cache.F2, 1.0 / 6.0, gt, equations, dg, 2, i, j,
                                   element)
        ...
        multiply_add_to_node_vars!(du, -1.0 / inv_jacobian, S_node, equations,
                                   dg, i, j, element)
        multiply_add_to_node_vars!(u_np1_low, dt, S_node_low, equations, dg, i, j, element)
...
@inline function mdrk_kernel_2!(du, u, us, t, dt, tolerances, element,
                                mesh::Union{StructuredMesh{2}, UnstructuredMesh2D,
                                            P4estMesh{2}},
                                nonconservative_terms::False, source_terms, equations,
                                dg::DGSEM, cache, alpha = true)
    ...
    Ftilde, Gtilde, F, G, ut, ust, U, S, u_np1 = cell_arrays[id]

    Ftilde, Gtilde, F, G, ut, ust, U, S, u_np1, u_np1_low = cell_arrays[id]
    u_np1_low .= u_element
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        _, _, cv_f, cv_g = contravariant_flux(u_node, Ja, equations)
        _, _, cv_fs, cv_gs = contravariant_flux(us_node, Ja, equations)
        ...
        # F_low = 1.0/3.0 * cv_f + 2.0/3.0 * cv_fs + 1.0/6.0 * ftilde_t
        # G_low = 1.0/3.0 * cv_g + 2.0/3.0 * cv_gs + 1.0/6.0 * gtilde_t

        F_low = Ftilde_node - 1.0 / 3.0 * ftilde_t
        G_low = Gtilde_node - 1.0 / 3.0 * gtilde_t
    ...
    for j in eachnode(dg), i in eachnode(dg)
        ...
        u_np1_low_node = Trixi.get_node_vars(mdrk_cache.u_np1_low, equations, dg, i, j,
                                             element)
        ...
        for v in eachvariable(equations)
            temporal_errors[element] += ((u_np1_node[v] - u_np1_low_node[v])
                                         ...
                                         (abstol +
                                          reltol *
                                          max(abs(u_np1_node[v]), abs(u_np1_low_node[v]))))^2
        end
    end
