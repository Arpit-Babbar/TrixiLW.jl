...
@inline function compute_and_load_temporal_errors!(u_ode, u_low_ode, semi, epsilon, ndofs,
                                                   tolerances, n_fail_it)
    ...
    for element in eachelement(dg, cache)
        for j in eachnode(dg), i in eachnode(dg)
            ...
            for v in eachvariable(equations)
                ...
                normalizing_factor = abstol +
                                     reltol * max(abs(u_node[v]), abs(u_low_node[v]))
...
function perform_step!(integrator, limiters, callbacks, lw_update,
                       time_step_computation::Adaptive,
...
function perform_step!(integrator, limiters, callbacks, lw_update,
                       time_step_computation::Adaptive, time_discretization::MDRK,
                       n_fail_it = 0)
    semi = integrator.p
