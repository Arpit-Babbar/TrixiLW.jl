using Trixi: UnstructuredMesh2D, P4estMesh, calc_surface_integral!, apply_jacobian!,
             get_surface_normal, get_one_sided_surface_node_vars, False
...
@muladd begin
    ...
    function prolong2interfaces!(cache, u,
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for v in eachvariable(equations), i in eachnode(dg)
                interface_cache.fn_low[1, v, i, interface] = fn_low[v, i, primary_side,
                                                                    primary_element]
                interface_cache.fn_low[2, v, i, interface] = fn_low[v, i, secondary_side,
                                                                    secondary_element]
            end

            if primary_side == 1
                for i in eachnode(dg)
                    outward_direction = get_surface_normal(normal_directions, i,
                                                           primary_side,
                                                           primary_element)
            ...
            elseif primary_side == 2
                for i in eachnode(dg)
                    outward_direction = get_surface_normal(normal_directions, i,
                                                           primary_side,
                                                           primary_element)
                    u_node = Trixi.get_node_vars(u, equations, dg, nnodes(dg), i,
                                                 primary_element)
                    U_node = Trixi.get_node_vars(U, equations, dg, nnodes(dg), i,
                                                 primary_element)
            ...
            elseif primary_side == 3
                for i in eachnode(dg)
                    outward_direction = get_surface_normal(normal_directions, i,
                                                           primary_side,
                                                           primary_element)
                    u_node = Trixi.get_node_vars(u, equations, dg, i, nnodes(dg),
                                                 primary_element)
                    U_node = Trixi.get_node_vars(U, equations, dg, i, nnodes(dg),
                                                 primary_element)
    ...
    function calc_interface_flux!(surface_flux_values, dt,
        ...
        @threaded for interface in eachinterface(dg, cache)
            ...
            for primary_index in eachnode(dg)
                ...
                f_ll = get_one_sided_surface_node_vars(f, equations, dg, 1, primary_index,
                                                       interface)
                ual = get_one_sided_surface_node_vars(u, equations, dg, 1, primary_index,
                                                      interface)
                u_ll = get_one_sided_surface_node_vars(U, equations, dg, 1, primary_index,
                                                       interface)
                fn_inner_ll = get_one_sided_surface_node_vars(fn_low, equations, dg, 1,
                                                              primary_index, interface)

                f_rr = get_one_sided_surface_node_vars(f, equations, dg, 2, secondary_index,
                                                       interface)
                uar = get_one_sided_surface_node_vars(u, equations, dg, 2, secondary_index,
                                                      interface)
                u_rr = get_one_sided_surface_node_vars(U, equations, dg, 2, secondary_index,
                                                       interface)
                fn_inner_rr = get_one_sided_surface_node_vars(fn_low, equations, dg, 2,
                ...
                for v in eachvariable(equations)
                    surface_flux_values[v, primary_index, primary_side, primary_element] = (alp *
                                                                                            fn[v] +
                                                                                            (1.0 -
                                                                                             alp) *
                                                                                            Fn[v])
                    surface_flux_values[v, secondary_index, secondary_side, secondary_element] = -(alp *
                                                                                                   fn[v] +
                                                                                                   (1.0 -
                                                                                                    alp) *
                                                                                                   Fn[v])
        ...
        @threaded for boundary in eachboundary(dg, cache)
            ...
            elseif side == 2
                for node_index in eachnode(dg)
                    outward_direction = get_surface_normal(normal_directions, node_index,
                                                           side, element)
                    u_node = get_node_vars(U, equations, dg, nnodes(dg), node_index,
                                           element)
                    f_node = get_flux_vars(F, equations, dg, nnodes(dg), node_index,
                                           element)
                    fn_node = normal_product(f_node, equations, outward_direction)
                    # f_node = Trixi.flux(u_node, outward_direction, equations)
                    for v in eachvariable(equations)
            ...
            elseif side == 3
                for node_index in eachnode(dg)
                    outward_direction = get_surface_normal(normal_directions, node_index,
                                                           side, element)
                    u_node = get_node_vars(U, equations, dg, node_index, nnodes(dg),
                                           element)
                    f_node = get_flux_vars(F, equations, dg, node_index, nnodes(dg),
                                           element)
                    fn_node = normal_product(f_node, equations, outward_direction)
                    # f_node = Trixi.flux(u_node, outward_direction, equations)
                    for v in eachvariable(equations)
    ...
    function calc_boundary_flux!(cache, t, dt, boundary_conditions,
        ...
        calc_boundary_flux_by_type!(cache, t, dt, boundary_condition_types,
                                    boundary_indices,
                                    mesh, equations, surface_integral, time_discretization,
                                    dg, scaling_factor)
        return nothing
    end
        ...
        @threaded for local_index in eachindex(boundary_indexing)
            ...
            for node in eachnode(dg)
                calc_boundary_flux!(surface_flux_values, t, boundary_condition,
                                    ...
                                    equations, surface_integral, dt, time_discretization,
                                    dg, cache,
                                    node, side, element, boundary, scaling_factor)
            end
        end
    end
