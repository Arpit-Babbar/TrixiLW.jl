function load_inverse_jacobian!(inverse_jacobian, mesh::UnstructuredMesh2D, interface_range,
    ...
    for interface in eachinterface(dg, cache)
        ...
        for i in 1:n_nodes
            if primary_side == 1
                inverse_jacobian[i, interface] = elements.inverse_jacobian[i, 1,
                                                                           primary_element]
            elseif primary_side == 2
                inverse_jacobian[i, interface] = elements.inverse_jacobian[n_nodes, i,
                                                                           primary_element]
            elseif primary_side == 3
                inverse_jacobian[i, interface] = elements.inverse_jacobian[i, n_nodes,
                                                                           primary_element]
            elseif primary_side == 4
                inverse_jacobian[i, interface] = elements.inverse_jacobian[1, i,
                                                                           primary_element]
            else
...
function create_boundary_cache(mesh::Union{UnstructuredMesh2D, P4estMesh{2}}, equations, dg,
                               uEltype, RealT,
