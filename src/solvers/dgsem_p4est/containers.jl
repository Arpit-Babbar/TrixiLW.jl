function load_inverse_jacobian!(inverse_jacobian, mesh::P4estMesh{2}, interface_range, dg,
                                cache)
    n_nodes = nnodes(dg)
    ...
    for interface in interface_range
        ...
        for i in 1:n_nodes
            inverse_jacobian[i, interface] = elements.inverse_jacobian[i_primary, j_primary,
                                                                       primary_element]
...
mutable struct L2MortarContainer_lw_P4est{RealT, uEltype <: Real, NDIMS, NDIMSP3, Temp} <:
               AbstractContainer
...
function create_mortar_cache(mesh::P4estMesh, equations, dg, uEltype, RealT, cache,
