...
function prolong2interfaces!(cache, u,
    ...
    @threaded for interface in eachinterface(dg, cache)
        ...
        for v in eachvariable(equations), i in eachnode(dg)
            interface_cache.fn_low[1, v, i, interface] = fn_low[v, i, primary_direction,
                                                                primary_element]
            interface_cache.fn_low[2, v, i, interface] = fn_low[v, i, secondary_direction,
                                                                secondary_element]
        end
    end
...
@inline function calc_interface_flux!(surface_flux_values,
    ...
    u_ll, u_rr = get_surface_node_vars(u, equations, dg, primary_node_index,
                                       interface_index)
    f_ll, f_rr = get_surface_node_vars(f, equations, dg, primary_node_index,
                                       interface_index)
    fn_inner_ll, fn_inner_rr = get_surface_node_vars(fn_low, equations, dg,
...
function compute_alp(u_ll, u_rr, primary_element_index, secondary_element_index, Jl, Jr, dt,
                     fn, Fn, fn_inner_ll, fn_inner_rr, primary_node_index, equations, dg,
...
function get_blended_flux(u_ll, u_rr, primary_element_index, secondary_element_index, Jl,
                          Jr, dt,
...
function calc_boundary_flux!(cache, t, dt, boundary_condition::BC,
                             boundary_indexing::Vector,
                             mesh::P4estMesh{2},
                             equations, surface_integral,
                             time_discretization::AbstractLWTimeDiscretization, dg::DG,
...
@inline function calc_boundary_flux!(surface_flux_values, t, dt, boundary_condition,
    ...
    u_inner = Trixi.get_node_vars(boundary_cache.u, equations, dg, node_index,
                                  boundary_index)
    U_inner = Trixi.get_node_vars(boundary_cache.U, equations, dg, node_index,
                                  boundary_index)
    f_inner = Trixi.get_node_vars(boundary_cache.f, equations, dg, node_index,
                                  boundary_index)
    ...
    flux_ = boundary_condition(U_inner, f_inner, u_inner, outer_cache, normal_direction, x,
                               t, dt,
...
function prolong2mortars!(cache, u,
    ...
    @threaded for mortar in eachmortar(dg, cache)
        ...
        for position in 1:2
            ...
            for i in eachnode(dg)
                ...
                for v in eachvariable(equations)
                    cache.mortars.u[1, v, position, i, mortar] = u[v, i_small, j_small,
                                                                   element]
                    lw_mortars.U[1, v, position, i, mortar] = U[v, i_small, j_small,
                                                                element]
            ...
            for v in eachvariable(equations), i in eachnode(dg)
                lw_mortars.fn_low[1, v, position, i, mortar] = fn_low[v, i, small_direction,
                                                                      element]
            end
        end
        ...
        for i in eachnode(dg)
            ...
            normal_direction = -0.5 *
                               get_normal_direction(large_direction, contravariant_vectors,
        ...
        for position in 1:2
            ...
            for node in eachnode(dg)
                ...
                calc_mortar_flux!(fstar, mesh, nonconservative_terms, equations, dt,
                                  ...
                                  mortar, position, element, element_large,
                                  normal_direction,
                                  node)
...
@inline function calc_mortar_flux!(fstar,
                                   ...
                                   time_discretization::AbstractLWTimeDiscretization,
                                   dg::DG, cache,
                                   mortar_index, position_index, element_small,
                                   element_large,
    ...
    f_ll, f_rr = get_surface_node_vars(F, equations, dg, position_index, node_index,
                                       mortar_index)
    fn_inner_ll, fn_inner_rr = get_surface_node_vars(fn_low, equations, dg, position_index,
    ...
    alp = compute_alp(u_ll, u_rr, element_small, element_large,
                      Jl, Jr, dt, fn, Fn, fn_inner_ll, fn_inner_rr, node_index, equations,
                      dg, dg.volume_integral)
