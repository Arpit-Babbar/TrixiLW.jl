import Trixi: init_mpi_interfaces, init_mpi_interface_node_indices!, nmpiinterfaces, nmpimortars,
              init_mpi_mortars

using Trixi: count_required_surfaces, init_surfaces!, init_mpi_interfaces!, P4estMPICache,
             ParallelP4estMesh, P4estMPIInterfaceContainer, init_mpi_mortars!, P4estMPIMortarContainer
...
function init_mpi_mortars(mesh::ParallelP4estMesh, equations,
    ...
    # function from trixi initializes some connectivity information for which we want to
    # use the Trixi function without duplication
    mpi_mortars_ = P4estMPIMortarContainer{NDIMS, uEltype, RealT, NDIMS + 1, NDIMS + 2,
                                          NDIMS + 3}(u, local_neighbor_ids,
                                                     ...
                                                     _normal_directions)
    if n_mpi_mortars > 0
        init_mpi_mortars!(mpi_mortars_, mesh, basis, elements)
    end

    # Now we just move what we get from the Trixi function into our own container
    mpi_mortars = P4estMPIMortarContainerLW{NDIMS, uEltype, RealT, NDIMS + 1, NDIMS + 2,
                                          NDIMS + 3}(mpi_mortars_.u,
                                                     U, F,
                                                     mpi_mortars_.local_neighbor_ids,
                                                     mpi_mortars_.local_neighbor_positions,
                                                     mpi_mortars_.node_indices,
                                                     mpi_mortars_.normal_directions,
                                                     mpi_mortars_._u,
                                                     _U, _F, 
                                                     mpi_mortars_._node_indices,
                                                     mpi_mortars_._normal_directions)
    return mpi_mortars
end
