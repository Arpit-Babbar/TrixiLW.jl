...
function rhs!(du_ode, u_ode, semi::SemidiscretizationHyperbolic, t,
    ...
    @trixi_timeit timer() "rhs!" rhs!(du, u,
                                      t, cache.dt[1], mesh, equations, initial_condition,
...
function rhs_mdrk1!(du_ode, u_ode, semi::SemidiscretizationHyperbolic, t,
                    tolerances = (; abstol = 0.0, reltol = 0.0))
    @unpack mesh, equations, initial_condition, boundary_conditions, source_terms, solver, cache = semi
    ...
    @trixi_timeit timer() "rhs!" rhs_mdrk1!(du, u,
                                            t, mesh, equations, initial_condition,
                                            boundary_conditions, source_terms, solver,
                                            get_time_discretization(solver),
                                            cache, tolerances)
    runtime = time_ns() - time_start
...
function rhs_mdrk2!(du_ode, u_ode, semi::SemidiscretizationHyperbolic, t,
                    tolerances = (; abstol = 0.0, reltol = 0.0))
    @unpack mesh, equations, initial_condition, boundary_conditions, source_terms, solver, cache = semi
...
function SemidiscretizationHyperbolic(mesh,
    ...
    # From TrixiLW.jl
    cache = create_cache(mesh, equations, time_discretization, solver, RealT, uEltype,
                         cache)
