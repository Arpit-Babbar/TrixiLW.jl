...
function resize_element_cache!(mesh::Union{StructuredMesh, UnstructuredMesh2D}, equations,
                               solver, cache)
    nothing
end
...
function resize_element_cache!(mesh::Union{TreeMesh, P4estMesh}, equations, solver, cache)
    ...
    element_cache.U = unsafe_wrap(Array, pointer(_U),
                                  (n_variables, ntuple(_ -> n_nodes, NDIMS)..., n_elements))
    element_cache.F = unsafe_wrap(Array, pointer(_F),
                                  (n_variables, NDIMS, ntuple(_ -> n_nodes, NDIMS)...,
                                   n_elements))
    element_cache.fn_low = unsafe_wrap(Array, pointer(_fn_low),
                                       (n_variables, ntuple(_ -> n_nodes, NDIMS - 1)...,
                                        2^NDIMS, n_elements))
    ...
    if isa(get_time_discretization(solver), MDRK) # TODO - Multiple dispatch instead?
        ...
        element_cache.mdrk_cache.us = unsafe_wrap(Array, pointer(_us),
                                                  (n_variables,
                                                   ntuple(_ -> n_nodes, NDIMS)...,
                                                   n_elements))
        element_cache.mdrk_cache.u_np1 = unsafe_wrap(Array, pointer(_unp1),
                                                     (n_variables,
                                                      ntuple(_ -> n_nodes, NDIMS)...,
                                                      n_elements))
        element_cache.mdrk_cache.u_np1_low = unsafe_wrap(Array, pointer(_unp1_low),
                                                         (n_variables,
                                                          ntuple(_ -> n_nodes, NDIMS)...,
                                                          n_elements))
        element_cache.mdrk_cache.U2 = unsafe_wrap(Array, pointer(_U2),
                                                  (n_variables,
                                                   ntuple(_ -> n_nodes, NDIMS)...,
                                                   n_elements))
        element_cache.mdrk_cache.S2 = unsafe_wrap(Array, pointer(_S2),
                                                  (n_variables,
                                                   ntuple(_ -> n_nodes, NDIMS)...,
                                                   n_elements))
        element_cache.mdrk_cache.F2 = unsafe_wrap(Array, pointer(_F2),
                                                  (n_variables, NDIMS,
                                                   ntuple(_ -> n_nodes, NDIMS)...,
                                                   n_elements))
    end
...
function resize_mortar_cache!(mesh::Union{StructuredMesh, UnstructuredMesh2D}, equations,
                              dg, cache)
    nothing
end
...
function resize_mortar_cache!(mesh::TreeMesh, equations, dg, cache)
    ...
    resize_mortar_array!.((_U_upper, _U_lower, _F_upper, _F_lower, _fn_low_upper,
                           _fn_low_lower))
    ...
    lw_mortars.fn_low_lower = wrap_.((_U_upper, _U_lower, _F_upper, _F_lower, _fn_low_upper,
                                      _fn_low_lower))
...
function resize_mortar_cache!(mesh::P4estMesh{2}, equations, dg, cache)
    ...
    lw_mortars.inverse_jacobian = unsafe_wrap(Array{RealT, NDIMS},
                                              pointer(_inverse_jacobian),
                                              (n_nodes, n_mortars))

    load_inverse_jacobian!(lw_mortars.inverse_jacobian, mesh, eachmortar(dg, cache), dg,
                           cache)
