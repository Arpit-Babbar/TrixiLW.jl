...
plt.figure()
plt.scatter(lower[:, 1] .- 38.0, lower[:, 2], label = "\$ y > 0 \$", marker = "s",
            facecolors = "none", edgecolors = "red")
plt.scatter(upper[:, 1] .- 38.0, upper[:, 2], s = 70, label = "\$ y < 0 \$", marker = "o",
            facecolors = "none", edgecolors = "g")
plt.xlim(max(minimum(lower[:, 1]), minimum(upper[:, 1])) .- 38.1,
         maximum(lower[:, 1]) .- 37.9)
plt.legend()
plt.grid(true)
plt.xlabel("\$ x \$")
