...
function plot_python_ncells_vs_y(; legend = nothing,
                                 bflux = "",
                                 degree = 3, show = "yes",
                                 files_labels = (("ns_conv_mdrk.txt", "MDRK"),
                                                 ("ns_conv_lw.txt", "LW-$degree")),
    ...
    for i in 1:n_curves
        ...
        ax_error.loglog(error[:, 1], error[:, 2], marker = markers[i], c = colors[i],
                        mec = "k",
...
function plot_python_ndofs_vs_y(; legend = nothing,
    ...
    for i in 1:n_curves
        file, label = files_labels[i]
        error = readdlm(joinpath(dir, file))
        ax_error.loglog(error[:, 1] * (degree + 1), error[:, 2], marker = markers[i],
                        c = colors[i], mec = "k",
                        fillstyle = "none", label = label)
    end
    file_for_theo, _ = files_labels[1]
