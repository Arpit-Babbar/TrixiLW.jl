...
for polydeg in 3:3
   for level in 1:n_levels
      ...
      #                          tspan = (0.,1.0))
      local sol = trixi_include("$(TrixiLW.examples_dir_trixilw())/p4est_2d_dgsem/elixir_euler_isentropic_hennemann.jl",
                           initial_refinement_level = level,
   ...
   writedlm(joinpath(TrixiLW.base_dir(), "results/isentropic_$polydeg.txt"), lw_errors)
end
...
for polydeg in 3:3
   for level in 1:n_levels_p4est
      ...
      #                          tspan = (0.,1.0))
      local sol = trixi_include("$(TrixiLW.examples_dir_trixilw())/p4est_2d_dgsem/elixir_euler_isentropic_hennemann.jl",
                           refinement_level = level, polydeg = polydeg)
      l2_lw, linf_lw = analysis_callback(sol)
      # mdrk_errors[level] = l2_mdrk[1]
      lw_errors_p4est[level,1] = size(sol.u[1], 1) / (polydeg+1)^2
      lw_errors_p4est[level, 2:end] .= l2_lw
   end
   writedlm(joinpath(TrixiLW.base_dir(), "results/isentropic_$polydeg.txt"), lw_errors_p4est)
end
