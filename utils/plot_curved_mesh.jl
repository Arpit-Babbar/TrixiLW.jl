...
function plot_mesh(semi, mesh::P4estMesh{2}, dg::DG; nvisnodes = 10,
    ...
    interpolation_matrix = polynomial_interpolation_matrix(get_nodes(basis),
                                                           LinRange(-1.0, 1.0, nvisnodes))
    ...
    for interface in eachinterface(dg, cache)
        ...
        for i in eachnode(dg)
            x = get_node_coords(node_coordinates, equations, dg, i_primary, j_primary,
                                primary_element)
    ...
    for mortar in eachmortar(dg, cache)
        ...
        for position in 1:2
            ...
            for i in eachnode(dg)
                x = get_node_coords(node_coordinates, equations, dg, i_small, j_small,
                                    element)
            ...
            if is_in_limit(nodes_array, xlimits, ylimits)
                @views plotter!(p,
                                interpolation_matrix * nodes_array[1, :],
                                interpolation_matrix * nodes_array[2, :], color = :black,
                                linewidth = linewidth, label = false)
            end
        end
    end
    ...
    for boundary in eachboundary(dg, cache)
        ...
        if is_in_limit(nodes_array, xlimits, ylimits)
            @views plotter!(p, nodes_array[1, :], nodes_array[2, :], color = :black,
