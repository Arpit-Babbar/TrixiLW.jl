...
if elixir_name == "elixir_advection_diffusion_diff02.jl"
    ...
    if prod(advection_velocity .≈ (1.5, 1.0)) == true
        writedlm(joinpath(TrixiLW.base_dir(), "results/adv_diff_lw.txt"),
                 zip(nelems, lw_errors))
        writedlm(joinpath(TrixiLW.base_dir(), "results/adv_diff_mdrk.txt"),
                 zip(nelems, mdrk_errors))
    else
        @assert prod(advection_velocity .≈ (0.0, 0.0)) == true
        writedlm(joinpath(TrixiLW.base_dir(), "results/heat_lw.txt"),
                 zip(nelems, lw_errors))
        writedlm(joinpath(TrixiLW.base_dir(), "results/heat_mdrk.txt"),
                 zip(nelems, mdrk_errors))
    end
elseif elixir_name == "elixir_advection_diffusion.jl"
    @assert elixir_name == "elixir_advection_diffusion.jl"
    @assert diffusivity() ≈ 1e-06
    @assert prod(advection_velocity .≈ (1.5, 1.0)) == true
    ...
    writedlm(joinpath(TrixiLW.base_dir(), "results/adv_diff_nonperiodic_lw.txt"),
             zip(nelems, lw_errors))
    writedlm(joinpath(TrixiLW.base_dir(), "results/adv_diff_nonperiodic_mdrk.txt"),
             zip(nelems, mdrk_errors))
end
