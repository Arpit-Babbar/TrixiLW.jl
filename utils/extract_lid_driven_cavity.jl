...
sol_error_lw = sol
pd_y_error = PlotData1D(sol_error_lw.u[1], semi, slice = :y, point = (0.5, 0.5),
                        nvisnodes = 3)
...
pd_x_error = PlotData1D(sol_error_lw.u[1], semi, slice = :x, point = (0.5, 0.5),
                        nvisnodes = 3)
...
p = plot(error_lw_x[:, 2], error_lw_x[:, 1], width = 3, label = "LW error-based",
         size = (500, 400))
plot!(p, cfl_x[:, 2], cfl_x[:, 1], width = 3, label = "LW CFL-based", linestyle = :dash)
plot!(p, mdrk_x[:, 2], mdrk_x[:, 1], width = 3, label = "MDRK error-based",
      linestyle = :dashdot)
exact_data_y = readdlm("data/lid_driven_cavity_y_vs_u1_r1000.txt")
plot!(p, exact_data_y[:, 2], exact_data_y[:, 1], seriestype = :scatter, width = 3,
      label = "Ghia et al.", color = :black)
xlabel!(p, "\$ v_x \$")
ylabel!(p, "\$ y \$")
savefig(p, "data/y_vs_vx.pdf")
display(p)

p = plot(error_lw_y[:, 2], error_lw_y[:, 1], width = 3, label = "LW error-based",
         size = (500, 400))
plot!(p, cfl_y[:, 2], cfl_y[:, 1], width = 3, label = "LW CFL-based", linestyle = :dash)
plot!(p, mdrk_y[:, 2], mdrk_y[:, 1], width = 3, label = "MDRK error-based",
      linestyle = :dashdot)
exact_data_x = readdlm("data/lid_driven_cavity_x_vs_u2_r1000.txt")
plot!(p, exact_data_x[:, 2], exact_data_x[:, 1], seriestype = :scatter, color = :black,
      width = 3, label = "Ghia et al.")
xlabel!(p, "\$ v_y \$")
ylabel!(p, "\$ x \$")
# plot!(flip = true)
